...
class CartesianTaskPlanner {
 ...
 public:
  ...
  float planCartesianMotionTask(MoveGroupInterface* move_group,
    ...
    ROS_INFO(
        "Cartesian task planning for [%s]:\n Starting Poses: %d  \n Final "
        "Poses: %d \n Max IK solutions per pose: %d",
        move_group->getName().c_str(), (int)start_poses.size(),
        (int)final_poses.size(), max_configurations);
    ros::Time start_time = ros::Time::now();
    bool task_plan_found = false;
    motion_plans.clear();
    invalid_states.clear();
    ...
    // ROS_WARN("post %d", acm.getEntry(jmb->getLinkModelNames()[0],
    // eef_links_names[0])); Start planning
    MoveGroupInterface::Plan cartesian_motion_plan, approach_plan,
        best_approach_plan;

    typedef std::vector<geometry_msgs::Pose>::iterator PoseIter;
    // Iterate through start poses.
    for (PoseIter start_pose_iter = start_poses.begin();
         start_pose_iter != start_poses.end(); start_pose_iter++) {
      bool ik_solution_found = true;
      uint ik_configurations_count = 1;
      while (ik_solution_found) {
        auto ik_check_callback =
            boost::bind(&CartesianTaskPlanner::validateIKSolution, this, _1, _2, _3);
        ik_solution_found = cartesian_start_state.setFromIK(
            jmb, *start_pose_iter, 10, 0.01, ik_check_callback);
        // Check if IK solution is found AND if a motion plan to reach it has
        // been found
        if (ik_solution_found && validateJointSpaceTargetMotion(
                                     move_group, current_state,
                                     cartesian_start_state, &approach_plan)) {
          ...
          double fraction_of_plan = 0;
          // Iterate through final poses
          for (PoseIter final_pose_iter = final_poses.begin();
               final_pose_iter != final_poses.end(); final_pose_iter++) {
            std::vector<geometry_msgs::Pose> waypoints = {*final_pose_iter};
            fraction_of_plan = move_group->computeCartesianPath(
                waypoints, eef_step, jump_threshold, trajectory, true);
            if (fraction_of_plan > max_cartesian_motion) {  // Best plan so far
              ...
              ROS_INFO("Pose %d (IK %d) allows for %.1f%% of cartesian motion",
                       ...
                       ik_configurations_count, max_cartesian_motion * 100);
            }
            if (fraction_of_plan ==
                1)  // Current final goal is reachable through cartesian motion
              break;
          }
          move_group->setGoalOrientationTolerance(5 * M_PI / 180);
          srv.request.scene = grasp_ps_msg;
  ...
  float planCartesianMotionTask(MoveGroupInterface* move_group,
                                ...
                                const std::vector<std::string> allow_collision_objects,
                                uint max_configurations = 10) {
    std::vector<geometry_msgs::Pose> start_poses{start_pose},
        final_poses{final_pose};
    return planCartesianMotionTask(move_group, start_poses, final_poses,
